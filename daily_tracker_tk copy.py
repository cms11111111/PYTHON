...
class BudgetApp:
    def __init__(self, root):
        ...
        # [DEBUG 4] 
        # 1. 行高增加到 50 (再加大間距)
        # 2. 字體縮小為 Arial 8 (減少文字擁擠)
        style.configure("TNotebook.Tab", font=("Arial", 12), padding=[5, 8])
        style.configure("Treeview", font=("Arial", 8), rowheight=50) 
        style.configure("Treeview.Heading", font=("Arial", 10, "bold"))
        
        # 頂部概況
        self.sum_fr = tk.Frame(root, bg="#0078d7", pady=15)
        self.sum_fr.pack(fill="x")
        tk.Label(self.sum_fr, text="本月收支概況", bg="#0078d7", fg="white", font=("Arial", 14, "bold")).pack()
    ...
    def setup_add_tab(self):
        ...
        tk.Label(r1, text="日期:", font=("Arial", 12), width=5, anchor="w").pack(side="left")
        ...
        tk.Label(r2, text="金額:", font=("Arial", 12), width=5, anchor="w").pack(side="left")
        self.e_amt = tk.Entry(r2, font=("Arial", 14, "bold"), bg="#fffde7", width=12)
        self.e_amt.pack(side="left", padx=5) 
        self.e_amt.bind("<FocusIn>", self.clear_ok_msg)
        self.e_amt.bind("<Button-1>", self.clear_ok_msg)

        # 3. 用途
        r3 = tk.Frame(f); r3.pack(fill="x", pady=5)
        tk.Label(r3, text="用途:", font=("Arial", 12), width=5, anchor="w").pack(side="left")
        self.e_use = tk.Entry(r3, font=("Arial", 12), width=15)
        self.e_use.pack(side="left", padx=5) 
        ...
        self.c_btns = {}
        for c in cats:
            # [DEBUG 5] 修正按鈕寬度，解決"其他"按鈕跑出畫面問題
            # 1. width=3: 縮小按鈕實體寬度
            # 2. font=("Arial", 8): 縮小字體以配合按鈕寬度
            # 3. padx=0: 移除按鈕間的間距
            b = tk.Button(cat_fr, text=c, font=("Arial", 8), width=3,
                            command=lambda x=c: self.set_cat(x), relief="flat", bg="#e0e0e0")
            b.pack(side="left", expand=True, padx=0) 
            self.c_btns[c] = b

        tk.Label(f, text="", height=1).pack()
        self.btn_save = tk.Button(f, text="儲存記錄", bg="#0078d7", fg="white", font=("Arial", 14, "bold"), 
    ...
    def setup_history_tab(self):
        ...
        # [DEBUG 4] 調整欄位寬度 (Total ~240px)
        self.tree.heading("date", text="日期"); self.tree.column("date", width=75, anchor="center")
        self.tree.heading("cat", text="類別"); self.tree.column("cat", width=35, anchor="center")
        self.tree.heading("use", text="用途"); self.tree.column("use", width=75, anchor="center")
        self.tree.heading("amt", text="金額"); self.tree.column("amt", width=55, anchor="e")
    ...
    def setup_search_tab(self):
        ...
        # [DEBUG 4] 調整欄位寬度
        self.s_tree.heading("date", text="日期"); self.s_tree.column("date", width=75, anchor="center")
        self.s_tree.heading("cat", text="類別"); self.s_tree.column("cat", width=35, anchor="center")
        self.s_tree.heading("use", text="用途"); self.s_tree.column("use", width=75, anchor="center")
        self.s_tree.heading("amt", text="金額"); self.s_tree.column("amt", width=55, anchor="e")
